...
/* Rules ?
 ...
 CPU = PLL / 2^(PLL div)
 AHB clock = CPU / 2^(AHB div). Warning ! AHB need to by slowly synchronized with CPU using few steps, it is good idea to hide that steps
 Vdd = from 1 to 7. Max voltage=7. Only 3 bit numbers ! Can't be design overvoltage CPU 
*/
...
/* // It's trash code, how do you want to incerase CPU speed without reprograming PLL ?
 * //this is only 800MHz, not more, not less with unsynchronized AHB BUS clock with do also dropdown performace
...
/*
	...
	{ 0, 1152000, ACPU_PLL_2, 2, 0, 144000, 3, 7, 200000}, // hidden, need for stability
	{ 1, 1209600, ACPU_PLL_2, 2, 0, 151200, 3, 7, 200000}, // stable more or less i think


 * 	{ 0, 1305600, ACPU_PLL_2, 2, 0, 163200, 3, 7, 200000 },
	{ 0, 1401600, ACPU_PLL_2, 2, 0, 175000, 3, 7, 200000 },
	{ 1, 1536000, ACPU_PLL_2, 2, 0, 192000, 3, 7, 200000 },	



	{ 0, 1270000, ACPU_PLL_4, 6, 0, 158750, 3, 7, 200000, &pll4_cfg_tbl[3]},
	{ 0, 1340000, ACPU_PLL_4, 6, 0, 167500, 3, 7, 200000, &pll4_cfg_tbl[3]},
	{ 1, 1401600, ACPU_PLL_4, 6, 0, 175200, 3, 7, 200000, &pll4_cfg_tbl[3]},
...
/*	{ 1, 800000, ACPU_PLL_2, 2, 1, 100000, 3, 6, 200000 }, // performace the same like in 600MHz */
/* 		{ 1, 1200000, ACPU_PLL_2, 2, 0, 150000, 3, 7, 200000},  // unstable !
 * 	{ 1, 1500000, ACPU_PLL_2, 2, 0, 187500, 3, 7, 200000}, // for stability test !!!
	{ 1, 1050000, ACPU_PLL_2, 2, 0, 131250, 3, 7, 200000},
	{ 1, 1100000, ACPU_PLL_2, 2, 0, 137500, 3, 7, 200000}, // warning
	{ 1, 1150000, ACPU_PLL_2, 2, 0, 143750, 3, 7, 200000}, // posible ustable ????
 * 
 * */
...
static int acpuclk_set_vdd_level(int vdd)
{
	...
	*/
	...
	mb();
	udelay(62);
	if ((readl_relaxed(A11S_VDD_SVS_PLEVEL_ADDR) & 0x07) != vdd) {
    /*FIH-SW3-KERNEL-JC-S1VDDSwitch+[ */
#ifdef CONFIG_FIH_SEMC_S1
        vdderror++;
        if(vdderror > 50)
	...
	if (dynamic_reprogram) {
		if (tgt_s->pll == ACPU_PLL_4) {
			if (strt_s->pll == ACPU_PLL_4 ||
				...
				rc = pc_pll_request(backup_s->pll, 1);
				if (rc < 0) {
					pr_err("PLL%d enable failed (%d)\n",
						backup_s->pll, rc);
				}
				plls_enabled |= 1 << backup_s->pll;
...
static void __init acpu_freq_tbl_fixup(void)
{
	 ...
	 */
	for ( ; lst->tbl->a11clk_khz; lst->tbl++) {
		if (lst->tbl->pll_rate) {
			if (!dynamic_reprogram) {
				dynamic_reprogram = 1;
